...
import {MediaRail} from "@/pages/media/shared/mediaRails.tsx";
...
function extracted(align: "left" | "right", torrent: ContentTorrent, selected: Set<number>, onToggle: (idx: number) => void) {
	return (
		<div
			className={cn(
...
function TorrentProgressCard({
	...
	const popup = extracted(align, torrent, selected, onToggle);
		...
		<TorrentCard
			...
			actionSlot={
				downloaded ? (
					...
					<div
						className="grid size-[34px] place-items-center rounded-full bg-white/[0.16] font-mono text-[9px] font-bold text-white/80">
						{pct}
					</div>
				)
			}
            ...
            <span className={cn(media.rowTitle, "flex-1")} title={picked.title}>
              {picked.title}
					...
					{preview === "loading" ? (
                      ...
                      <span
	                      className="flex flex-1 cursor-default items-center justify-between gap-2 px-0.5 py-1 text-body font-medium text-ink">
                        {g.label}
                      </span>
