      ...
      {(() => {
        const heroItem = resume[0]
          ? {
              ...
              progress: resume[0].positionPct,
            }
          : shownLibrary[0]
            ? {
                id: shownLibrary[0].id,
                title: shownLibrary[0].name,
                type: shownLibrary[0].type as string,
                progress: undefined,
              }
            : null;
        ...
        const handleHeroPlay = () => {
          ...
          if (lib)
            nav(
              `/media/${lib.type === "Series" ? "series" : "movie"}/${lib.id}`,
            );
        };
        return (
          <div className={ms.libHero} onClick={handleHeroPlay}>
            ...
            <div className={ms.libHeroBody}>
              ...
              <h1 className={ms.libHeroTitle}>{heroItem.title}</h1>
              ...
              {heroItem.progress != null && (
                <div className={ms.progRow}>
                  <div className={ms.progTrack}>
                    <div
                      className={ms.progFill}
                      style={{
                        width: heroItem.progress + "%",
                        background: accent,
                      }}
                    />
                  </div>
                  <span className={ms.progLabel}>
                    {Math.round(heroItem.progress)}% просмотрено
                  </span>
                </div>
              )}
            </div>
                  ...
                  >
                    ...
                    {it.played && (
                      <span
                        className={cn(
                          ms.posterBadge,
                          "left-[9px] right-auto bg-accent text-black",
                        )}
                        style={{
                          top: 9,
                        }}
